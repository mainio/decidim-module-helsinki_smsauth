...
shared_examples "authenticate with phone process" do
  ...
  describe "authentication process" do
    it "does the authentication process" do
      ...
      expect(page).to have_no_current_path(decidim_helsinki_smsauth.users_auth_sms_edit_path, wait: 10)
      expect(page).to have_no_content("Failed to verify the phone number.")
    end
  end
end
